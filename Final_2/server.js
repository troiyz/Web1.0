...
app.post('/confirmation', isAuthenticated, isAdmin, (req, res) => {
    ...
    console.log("new address for voter : ", newAddress);
        ...
        .then(doc => {
            Ballot.deployed().then(function (instance) {
                ...
                if (!doc.exists) {
                    ...
                    checkVote = instance.checkVote.call({ from: address });
                    checkVote.then(function (already) {
                        console.log(already);
                        if (already == false) {
                            nameProposals.then(function (resultName) {
                                var stringName = [];
            ...
            if (!doc.exists) {
                ...
                Ballot.deployed().then(function (instance) {
                    ...
                    voting.then(function (voteScore) {
                        console.log("Transaction ID vote =>: ", voteScore);
                        res.redirect('/thankyou')
                    }).catch(function (err) {
                        console.log(err);
                        res.json({ status: "failed to voted" });
                    })
                }).catch(err => {
                ...
                titleName.then(function (nameTitle) {
                    ...
                    timeStart.then(function (startTime) {
                        ...
                        timeEnd.then(function (endTime) {
                            ...
                            startTime = startTime.toNumber()
                            endTime = endTime.toNumber()
                            res.render('thankyou.ejs', { resultName, nameTitle, resultVoteCount, startTime, endTime });
                        })
                    })
                })
            })
